...
\* -------------------------------------------------------------------------- */
if( !function_exists('wps_remove_dashboard_widgets') ) :
...
function set_admin_menu_separator() {
  add_admin_menu_separator(50); // Where we'll put Custom Post Types
  add_admin_menu_separator(55); // Where we'll put Custom Post Types
}
 ...
 * @return string
 */
if( !function_exists('wps_make_filename_lowercase') ) :
  function wps_make_filename_lowercase($filename) {
      $info = pathinfo($filename);
      $ext  = empty($info['extension']) ? '' : '.' . $info['extension'];
      $name = basename($filename, $ext);
      return strtolower($name) . $ext;
  }
  add_filter('sanitize_file_name', 'wps_make_filename_lowercase', 10);
endif;